...
Panopticon query text and wrappers expand q-literal macros such as `{TimeWindowStart}`, `{TimeWindowEnd}`, `{Snapshot}`, `{FocusTime}`, `$TimeWindowStart`, `{TimeWindowStart:yyyy-MM-dd HH:mm:ss}`, `{IntervalMs}`, `{MaxDataPoints}`, `{RefID}`, and `{UserLogin}`. Panopticon dashboard parameters such as `{symbol}` and `{symbol:,}`, plus standard Grafana `$word`, `[[word]]`, and `${word}` forms, accept only conservative single-token external values: dotted ASCII identifiers without a leading dot, finite decimal/exponent numbers, Grafana interval tokens, and explicit ISO/q date/time shapes. Values containing quotes, slash comments, backslashes, whitespace/control bytes, Unicode, separators, or other q syntax fail before dispatch; the frontend never strips or escapes them. Resolved authored formats, field paths, registry macros, custom-All values, scene interpolation, and malformed or inconsistent Grafana interpolation audits fail closed rather than bypassing the formatter; unresolved references may remain literal. Arrays contain 1–1024 independently validated values of at most 256 bytes, each replacement is capped at 64 KiB, placeholder and dashboard-variable scans at 4,096 entries, and the final template at 256 KiB. Panopticon parameter names are capped at 128 bytes. A Panopticon delimiter is trusted query-authored syntax, but is capped at 32 UTF-8 bytes and cannot contain braces or control characters. Reserved dollar macros use the Go backend's substring-prefix behavior, so `$TimeWindowStartSuffix` is protected for backend expansion rather than treated as a Grafana variable. `Pano Wrapper` rewrites query text with one `{Query}` placeholder. `Pano Fn` calls a q function or lambda with the full request dictionary instead of evaluating query text directly.
