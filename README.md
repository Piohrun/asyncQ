...
Panopticon dashboard parameters are also expanded in Panopticon mode when a Grafana variable with the same name exists. For example, a Grafana variable named `symbol` lets a pasted query keep `{symbol}` or `{symbol:,}` syntax. Standard Grafana `$word`, `[[word]]`, and `${word}` substitutions use the same policy. Because the frontend cannot infer whether a placeholder is inside q text, a symbol, or an expression, it never quotes, escapes, strips, or otherwise rewrites an external value. Every scalar or multi-value element must already be one conservative ASCII token: a non-leading-dot identifier with nonempty identifier-like dotted segments, a finite decimal/exponent number, a Grafana interval (`ms`, `s`, `m`, `h`, `d`, `w`, `M`, or `y`), or an explicit ISO/q date, time, or timestamp. Quotes, slash comments, backslashes, whitespace/control bytes, Unicode, q separators/operators, and new expressions are rejected with an error before the query is sent. Safe built-ins such as `$__from`, `$__to`, `$__interval`, and `$__interval_ms` remain usable when their resolved value has one of those shapes. Missing variables and reserved backend macros remain literal.

Grafana also recognizes `[[word:format]]`, `${word:format}`, and `${word.fieldPath:format}`. AsyncQ rejects field paths and any resolved authored-format reference because those routes can override the validating formatter. It also rejects resolved registry macros, custom-All substitutions, scene interpolation, malformed adapters, and any interpolation whose fourth-argument Grafana audit record is missing, reordered, duplicated, or different from the formatter result. Unresolved references may remain literal. In Panopticon mode, `$TimeWindowStart`, `$TimeWindowEnd`, `$Snapshot`, and `$FocusTime` are protected with the same substring-prefix behavior as the Go backend. Consequently, a token such as `$TimeWindowStartSuffix` remains literal in the frontend so the backend can expand its `$TimeWindowStart` prefix; do not use Grafana variable names that begin with one of those reserved macros.

Standard Grafana multi-values are comma-joined. Panopticon multi-values use the delimiter after `:`, defaulting to `,`; that delimiter is query-author-controlled q syntax, not external dashboard data, so separators such as `{symbols: }` or `{symbols:;}` are permitted deliberately. Delimiters are nevertheless limited to 32 UTF-8 bytes and cannot contain braces or control characters. Parameter names are limited to 128 bytes; each replacement accepts 1–1024 values of at most 256 bytes each, up to 64 KiB total. At most 4,096 placeholders and 4,096 dashboard-variable metadata entries are scanned, and the final expanded q template is limited to 256 KiB. Put any required q quoting or casting in the authored template, for example ``sym=`{symbol}`` or ``sym in `$" " vs "{symbols: }"``.
