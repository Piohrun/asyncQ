...
For LLM-assisted migration checks, the AsyncQ-aware MCP tools can execute direct queries and dashboard panel targets. Sync targets run through Grafana's `/api/ds/query`; finite async targets (`async`, `pluginAsync`, `deferredAsync`, and `legacyAsync`) run through the datasource `POST async/run-and-wait` resource and return both the final frames and the async status timeline. `stream` targets can be inspected but not executed through this run-and-wait path because they are open Grafana Live subscriptions.

When panels use Grafana's Dashboard datasource, MCP `run_asyncq_panel_query` follows the dependent panel's `panelId` reference back to the source AsyncQ panel and returns `sourcePanelId`, `sourcePanelTitle`, `sourceTargetIndex`, and a warning describing the resolution. Use `check_business_suite_dashboard` for Business Table, Business Charts, and Business Forms dashboards: it validates Business Suite panel config, follows Dashboard datasource master-panel references, samples the backing AsyncQ data, and returns compact profile/cache diagnostics.

If the migration LLM has write access through the companion MCP server, prefer `create_asyncq_business_suite_dashboard` for this common pattern. It creates a Grafana 13 tabbed dashboard from a compact spec: each tab gets one AsyncQ master-data source panel, one diagnostics panel, and Business Charts/Table/Forms panels that reference the master panel through Grafana's Dashboard datasource. This avoids manually authoring large v2 dashboard JSON and generates the full Business Table v3 runtime option schema rather than the thin legacy defaults that can crash the Business Table plugin.

After API/data checks pass, run MCP `validate_dashboard_runtime` when a browser is available on the MCP host. It opens the dashboard with Playwright, walks tabs, and reports visible `No data`, Grafana/plugin crash text, page errors, serious JavaScript errors, and missing expected panel text. This is the check that catches Business plugin runtime failures that `run_asyncq_panel_query` and `check_business_suite_dashboard` cannot see. The tool requires Node.js plus Playwright; if Playwright is installed outside the MCP server working directory, pass `playwrightNodePath` or set `NODE_PATH`.
