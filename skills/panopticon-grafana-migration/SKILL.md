...
4. Choose AsyncQ settings.
   ...
   - Use `asyncq-masterdata-panel` as the source panel when the migrated dashboard should show data freshness, cache diagnostics, profile timings, or cache-control buttons. Its layout is stable across fresh queries and cache hits, so dependent diagnostic panels should not rely on cards appearing or disappearing.
   ...
   - Use `asyncq-excel-report-panel` for preconfigured workbook downloads. Report definitions belong in datasource `jsonData.excelReports`; the panel selects a report ID, sends the current dashboard time range, variables, and any frames returned to the report panel, shows progress while the backend writes the workbook, and downloads through a one-time datasource token. For large dedicated data sheets, set `writeMode: "stream"` on the report or binding and keep formulas/charts on separate template sheets pointing at the generated ranges.
