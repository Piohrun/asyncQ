...
func getConfig() (TestServerCfg, error) {
	...
	for _, v := range requiredCols {
		...
		if !pass {
			return cfg, fmt.Errorf("missing required column from config CSV: %s", v)
		}
	}
	cfg.Host = cfgMap["host"]
	cfg.Port, err = strconv.Atoi(cfgMap["port"])
	if err != nil {
		return cfg, fmt.Errorf("could not convert test port to int: %s", cfgMap["port"])
	}
	cfg.User = cfgMap["user"]
	cfg.Pass = cfgMap["pass"]
	i, err := strconv.Atoi(cfgMap["autoStart"])
	if err != nil {
		return cfg, fmt.Errorf("could not convert autoStart to int: %s", cfgMap["autoStart"])
	}
	cfg.autoStart = i > 0
	return cfg, nil
}
...
func TestOpenConnectionStd(t *testing.T) {
	...
	if err != nil {
		t.Skipf("Skipping live kdb+ connection test: %v", err)
		return
	}
